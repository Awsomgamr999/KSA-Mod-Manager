...
def require_kitten_path(name, prompt, required_exes=None):
    while True:
        ...
        # If EXE check is required
        if required_exes:
            found = False
            for exe in required_exes:
                if os.path.isfile(os.path.join(user_in, exe)):
                    found = True
                    break
            if not found:
                print(f"Path does not contain required file(s): {', '.join(required_exes)}")
                continue
...
def load_paths():
    ...
    try:
        ...
        manifest = paths.get("ManifestPath")
        game = paths.get("GamePath")
        mod_loader = paths.get("ModLoaderPath")

        return manifest, game, mod_loader
...
def check_for_updates():
    ...
    try:
        ...
        # Look for KSAModManager.zip specifically
        for asset in assets:
            name = asset.get("name", "").lower()
            if name == "ksamodmanager.zip":
                download_url = asset.get("browser_download_url")
                break

        if not download_url:
            print("No KSAModManager.zip found in release assets. Make sure you uploaded it.")
            return None, None
...
def install_update(download_url):
    ...
    if not os.path.exists(updater_path):
        print("ERROR: UpdateHelper.exe not found next to KSAModManager.exe")
        return

    print("Starting updater...")
    subprocess.Popen([updater_path, extract_dir, install_dir], close_fds=True)

    # Give Windows a moment to flush process termination so UpdateHelper sees KSAMM exit
    time.sleep(0.2)
    sys.exit(0)
...
def launch_game(game_path, mod_loader_path=None):
    ...
    # Common mod loader executable names
    mod_loader_candidates = ["StarMap.exe", "ModLoader.exe"]
    ...
    # Launch Mod Loader if found
    if mod_loader_exe:
        print(f"Launching game via Mod Loader at: {mod_loader_exe}")
        subprocess.Popen([mod_loader_exe], cwd=mod_loader_path)
        return

    # Otherwise launch normal game EXE
    if os.path.exists(game_exe):
        print(f"Launching game directly at: {game_exe}")
        subprocess.Popen([game_exe], cwd=game_path)
        return

    # Neither executable exists
    print("Error: Could not find game executable or mod loader.")
    ...
    while True:
        ...
        if choice == "1":
            ...
            manifest = require_kitten_path(
                "Manifest",
                "Enter path to the manifest (or type cancel):",
                required_exes=["manifest.toml"]
            )

            # Game directory must contain KSA.exe
            game_path = require_kitten_path(
                "Game directory",
                "Enter game directory (or type cancel):",
                required_exes=["KSA.exe"]
            )
            if not game_path:
                print("Game path is required.")
                continue

            # Ask if user wants to use a mod loader
            use_mod_loader = input("Use mod loader? (y/n): ").lower() == "y"
            if use_mod_loader:
                mod_loader_path = require_kitten_path(
                    "Mod Loader",
                    "Enter mod loader directory (or type cancel):",
                    required_exes=["StarMap.exe", "ModLoader.exe"]
                )
            else:
            ...
            if not game_path:
                print("Game path not set.")
                continue
            install_mods(manifest, game_path)
            check_for_dependencies(game_path)
